...
Парсер матчей за месяц. Функции:
 ...
 - сохранять результаты в Excel: data/YYYY/MM/matches_YYYY_MM.xlsx
   - лист 'matches'       — все строки (tournament, date, time, participants, stage, request_id)
...
Запуск из CLI:
    python MonthGrabbScheduler.py --year 2026 --month 3
    python MonthGrabbScheduler.py --year 2026 --month 3 --force
    python MonthGrabbScheduler.py --repair-day 2026-03-15 --year 2026 --month 3
    python MonthGrabbScheduler.py --test
"""
...
class ExcelMonthFile:
    # stage добавлен между participants и request_id
    MATCHES_HEADER = ["tournament", "date", "time", "participants", "stage", "request_id"]
    ...
    def replace_day_rows(self, date: str, new_rows: List[Dict]):
        ...
        with file_lock(self.lock_path):
            ...
            # Читаем всё, кроме удаляемого дня (col 2 = date)
            ...
            for i, r in enumerate(new_rows, start=start_row):
                ...
                ws.cell(row=i, column=5, value=r.get("stage", ""))       # col 5
                ws.cell(row=i, column=6, value=r.get("request_id", ""))  # col 6
...
class MonthGrabbScheduler:
    def __init__(self,
        ...
        if ChampionatGrabber is None:
            raise RuntimeError("ChampionatGrabber не доступен — положи ChampionatGrabber.py рядом с этим файлом")
    ...
    def _grab_with_retry(self, date_str: str, req_id: str) -> Dict:
        ...
        for attempt in range(1, self.retries + 1):
            ...
            try:
                ...
                grabber = ChampionatGrabber(timeout=self.timeout_per_day)
    ...
    def grab_one_day(self, date_str: str, force_grab: bool = True) -> int:
        ...
        if result["status"] == "ok":
            ...
            self.logger.info(
                f"grab_one_day done: {date_str} → {len(matches)} матчей",
                extra={"request_id": req_id}
            )
            return len(matches)
        else:
    ...
    def smoke_test(self, date_str: str = "2026-03-03"):
        """
        Быстрый тест: парсит один день, выводит первые 3 записи,
        проверяет наличие всех ожидаемых полей.
        """
        self.logger.info(f"smoke_test: {date_str}")
        req_id = gen_request_id()
        result = self._grab_with_retry(date_str, req_id)

        if result["status"] != "ok":
            self.logger.error(f"smoke_test FAILED: {result.get('error')}", extra={"alert": True})
            return False

        matches = result["matches"]
        expected_fields = {"tournament", "date", "time", "participants", "stage"}
        missing = [
            i for i, m in enumerate(matches[:5])
            if not expected_fields.issubset(m.keys())
        ]
        if missing:
            self.logger.error(f"smoke_test FAILED: отсутствуют поля в записях {missing}", extra={"alert": True})
            return False

        self.logger.info(f"smoke_test OK: {len(matches)} матчей, первые 3:")
        for m in matches[:3]:
...
def _parse_args():
    ...
    p.add_argument("--year",        type=int,  help="Год (обязателен без --test)")
    p.add_argument("--month",       type=int,  help="Месяц (обязателен без --test)")
    ...
    p.add_argument("--test",        action="store_true", help="Запустить smoke_test")
    p.add_argument("--test-date",   type=str,  default="2026-03-03", metavar="YYYY-MM-DD",
                   help="Дата для smoke_test (по умолчанию 2026-03-03)")
    return p.parse_args()
...
def main():
    ...
    if args.test:
        ok = sched.smoke_test(args.test_date)
        sys.exit(0 if ok else 1)

    if not args.year or not args.month:
        print("Укажите --year и --month (или используйте --test)")
        sys.exit(1)
